...
async function getStats(city) {
  ...
  fetch("https://api-nba-v1.p.rapidapi.com/teams?search=" + city, {
    method: "GET",
    headers: {
...
async function showStats(city1, city2) {
  var [firstResult, secondResult] = await Promise.all([
    getStats(city1),
    getStats(city2),
  ]);
  ...
  for (i = 0; i < Object.keys(stats1.response[0]).length; i++) {
    ...
    currentStat1El.textContent =
      Object.keys(stats1.response[0])[i] +
      ": " +
      Object.values(stats1.response[0])[i];
    currentStat2El.textContent =
      Object.keys(stats2.response[0])[i] +
      ": " +
      Object.values(stats2.response[0])[i];
